...
$('#busy').show();	
	...
	criter=criteria.value;
	 page_en_cours1=0;
   //alert(criter); 
 var request = { u: { login: window.localStorage.getItem("login"), pw: window.localStorage.getItem("pw"),urlOrg:window.localStorage.getItem("urlOrg"), entity: "activitypointer", critere: criter,page:0} };
...
cx=1;
  ...
  //  var criteria = document.getElementById('search');
	 ...
	 //criter=criteria.value;
   //alert(criter); 
 var request = { u: { login: window.localStorage.getItem("login"), pw: window.localStorage.getItem("pw"),urlOrg:window.localStorage.getItem("urlOrg") ,entity: "activitypointer", critere: criter,page:page_en_cours1} };
    var jsondata = JSON.stringify(request);
    //alert(jsondata);
    Type = "POST";
    Url = "http://"+sessionStorage.getItem("hostName")+"/WcfMobileHLI/Service1.svc/GetQuickSearch";
...
function ServiceSucceededQuickSerach(result)
{
  if (DataType == "json") {
        ...
        resultObject = result.GetQuickSearchResult;
		var div = document.getElementById("thelist"); // crée un élément <table>

        if (resultObject) {
		
		if(resultObject.Entities!=0)
		{
		var indice=0;
			 var id="";		
		//
...
elm2.innerHTML=" ";
			...
			if(i==0)
			{
		$('#thelist').append('<li style="height: 40;"><a href="javascript:goRecord('+indice+')"> <img src="imgActivities/'+objectType+'.png" style="margin: -1;margin-left: -10;margin-left: -1;margin-top: 10;" class="list-icon1" class="list-icon1"/><p class="line0" style="margin-left: 21;line-height: 0;height: 0;white-space: nowrap;margin-top: -5;"> '+subject + '</p> </a></li> <p> </p>');   
