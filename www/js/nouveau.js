...
function customeridLookup(entityLookupName)
{
 ...
 $('#liste1').append("<option >Selectionner l'entit\351</option>");
 ...
 $('#divModalDialog1').append('</SELECT>');
        ...
        {
 $('#liste1').append('<option value="lead">Pistes Commerciales</option>');

 $('#liste1').append('<option value="quote">Devis</option>');		
 ...
 $('#divModalDialog1').append('</SELECT>');
$('#divModalDialog1').append('<table style="width:100%" border="0" > <tr><td> <input id="search"  type="text"  placeholder="Rechercher" style=" margin-left: -100; margin: 10;width: 80%;display:inline-block;height:30;"  data-inline="true"/></td><td> <input href="" id="test" onclick="rechercher();"  style="width: 90%;" data-role="button" type="button" value="ok" data-inline="true"></input></td></tr> </table>');
...
$('#thelist').append('<li><a href="#" style="height: 30;margin-top: 10;" onClick="ModificationLookup('+ j +')"> <p class="line1" style="white-space: nowrap;">'+userdata.Entities[j].Attributes[0].value+'</p></a></li> ');     
...
if(taille_pagination1<6)
{
val =taille_pagination1-1;
var elm0=document.getElementById('n'+val);

elm0.innerHTML=" "+taille_pagination1+" ";
elm0.style.fontWeight = 'bold';
elm0.style.color='gray';

for(var v=0;v<taille_pagination1-1;v++)
{
var val=0;
...
for(var v=taille_pagination1+1;v<5;v++)
{
var elm2=document.getElementById('n'+v);
elm2.innerHTML=" ";
}
...
function ModificationLookup(i)
{
	 ...
	 //idLookup=Case[guid_emplacement];
	 sessionStorage.setItem(entityLookupName,idLookup);
	 //alert(userdata.Entities[i].Attributes[1].value);
...
var ent="";
	...
	if((interm[1]=="regardingobjectid")||(interm[3]=="customerid"))
	{
...
cx=1;
 var request = { u: { login: window.localStorage.getItem("login"), pw: window.localStorage.getItem("pw"),urlOrg:window.localStorage.getItem("urlOrg"), entity: ent, critere: criter,page:page_en_cours1} };
    var jsondata = JSON.stringify(request);
    Type = "POST";
    Url = "http://"+sessionStorage.getItem("hostName")+"/WcfMobileHLI/Service1.svc/GetQuickSearch";
    //alert(Url);   
...
div.innerHTML="";
		...
		if(cx1==0)
		{
		var page = document.getElementById("page"); // crée un élément <table>
page.innerHTML="";
		taille_total1=parseInt(userdata.EntityName);
		cx1=1;
		if(parseInt(taille_total1%list_par_page1)==0)
...
{
val =taille_pagination1-1;
var elm0=document.getElementById('@'+val);
...
if(v==taille_pagination1%nbre_page_par_bloc1-1)
{
val=taille_pagination1%nbre_page_par_bloc1-1;
var elm0=document.getElementById("@"+val);
val=(bloc_en_cours1-1)*nbre_page_par_bloc1+v+1;
elm0.innerHTML=" "+val+" ";
elm0.style.fontWeight = 'bold';
...
$('#busy').show();
        ...
        var request1 = {u: {login: sessionStorage.getItem('cleL'), pw: sessionStorage.getItem('cleP'),entity:sessionStorage.getItem("EntityName"), data: chaine_attribut} };
		chaine_attribut=" " ;
         var jsondata1 = JSON.stringify(request1);
	
        Type = "POST";
        Url = "http://"+sessionStorage.getItem("hostName")+"/WcfMobileHLI/Service1.svc/saveNewRecord";
        Data = jsondata1;
        ContentType = "application/json; charset=utf-8";
        DataType = "json";
        ProcessData = true;
...
$('#busy').hide();
    ...
    function ServiceSucceededNew(result) {
	
        if (DataType == "json") {
            var resultObject1 = result.saveNewRecordResult;
		
			$('#busy').hide();
            if (resultObject1) {
			if(resultObject1[21]=='/')
{
alert('un nouvel enregistrement est ajouté');
